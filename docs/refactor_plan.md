...
本计划基于 [现状分析](current_architecture.md)，规格来源为根目录 `docs-project_spec.md`。Phase 0–5 已完成；Phase 6–18 为待办。实际范围、验证与限制见 [progress.md](progress.md)。
...
在 Amap 类型化检索边界接入 Redis，键包含规范化参数与版本；按天气/POI/路线分类 TTL，验证过期、参数隔离、损坏值与 Redis 故障降级；不缓存失败为成功空值。
